...
import { NO_GROUPBY, type Aggregation, type AggregateCol, aggregateFunctions } from '../../data/aggregation';
...
export function AggregationLayer(props: AggregationLayerProps) {
  ...
  return (
    <div class={styles.AggregationLayer}>
      ...
      <label>
        Aggregate columns
        <Index each={columnList()}>{(col, i) => 
          <AggregationColumnRow value={col()} update={c => setColumn(c, i)} inputCols={props.aggregation.input.columnNames()} />
        }</Index>
      </label>
    </div>
  )
...
function AggregationColumnRow(props: { value: AggregateCol; update: (next: AggregateCol) => void; inputCols: string[] }) {
  const patch = (p: Partial<AggregateCol>) => props.update({ ...props.value, ...p });
  return (
    <SegmentedControl>
      <Input value={props.value.name} onChange={e => patch({ name: e.target.value })} />
      <Select 
